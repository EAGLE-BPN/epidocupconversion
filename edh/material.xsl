        ...
        <xsl:copy>
            ...
            <xsl:if test="text()">
                <xsl:variable name="voc_term">         
                    <xsl:choose>
                        <xsl:when test="document('eagle-vocabulary-material.rdf')//skos:prefLabel[lower-case(.)=lower-case(normalize-space($noquestion))]/parent::skos:Concept[not(ancestor::skos:exactMatch) or not(@rdf:about[contains(.,'archwort')])]/@rdf:about">
                            <xsl:variable name="seq" select="document('eagle-vocabulary-material.rdf')//skos:prefLabel[lower-case(.)=lower-case(normalize-space($noquestion))]/parent::skos:Concept[not(ancestor::skos:exactMatch) or not(@rdf:about[contains(.,'archwort')])]/@rdf:about"/>
                            <xsl:value-of select="$seq[1]"/>
                        </xsl:when>
                    <xsl:otherwise>
                        <xsl:variable name="seq" select="document('eagle-vocabulary-material.rdf')//skos:altLabel[lower-case(.)=lower-case(normalize-space($noquestion))]/parent::skos:Concept/@rdf:about"/>
