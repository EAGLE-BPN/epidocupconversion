...
<!--numbers-->
                        <xsl:analyze-string select="." regex="(\(\(%(\d+)\)\)\s*)+\(:(([a-zA-Z])*\s*([a-zA-Z])*)\)">
                            <xsl:matching-substring>
                                                                 
                                            <num>
                                       <xsl:attribute name="value">
                                           <xsl:value-of select="regex-group(2)"/>
<!--                                           when there are more values this is untrue: would need to sum the numeric values of each digit matched by \d-->
                                       </xsl:attribute>
                                                <xsl:value-of select="regex-group(3)"/>
                                            </num>
...
<!--@(obitus)         -->
          ...
          <!--orig-->      
                                                                      <xsl:analyze-string select="."
                                                                          regex="(ạ|ḅ|c̣|ḍ|ẹ|ḥ|ị|ḳ|ḷ|ṃ|ṇ|ọ|ṛ|ṣ|ṭ|ụ|ṿ|ẉ|ỵ|ẓ)">
<!--       underdots are a mess to find or I have no clue how to do it...     -->
                                                                          <xsl:matching-substring>
                                                                              <orig>
                                                                                  <xsl:value-of select="regex-group(1)"/>
                                                                              </orig>
                                                                          ...
                                                                          <xsl:non-matching-substring>
<!--    previously read   -->
                                                                              <xsl:analyze-string select="." regex="((\w+)(\p{{M}}+))">
                                                                                  <xsl:matching-substring>
                                                                                      
                                                                                      <supplied reason="undefined" evidence="previouseditor">
                                                                                          <xsl:value-of select="regex-group(2)"/>
                                         <!--      trying to get rid of the ̲   thing...                           <xsl:value-of select="translate(regex-group(3), '̲', '')"/>-->
                                                                                      </supplied>
                                                                                  </xsl:matching-substring>
                                                                                  <xsl:non-matching-substring>
